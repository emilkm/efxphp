...
class CommentParser
{
     ...
     * @var string
     */
    public $embeddedDataName = 'properties';
     ...
     */
    public function parse($comment, $isPhpDoc = true)
    {
        if (empty($comment)) {
            return $this->_data;
        }
        ...
        if ($isPhpDoc) {
            $comment = $this->removeCommentTags($comment);
        }

        $this->extractData($comment);
        return $this->_data;
    }
     ...
     */
    public function removeCommentTags($comment)
    {
        $pattern = '/(^\/\*\*)|(^\s*\**[ \/]?)|\s(?=@)|\s\*\//m';
    ...
    private function extractData($comment)
    {
        ...
        $comment = str_replace(
            array(self::ESCAPED_COMMENT_END, self::ESCAPED_AT_CHAR),
            array('*/', '@'),
            $comment
        );
        ...
        $d2 += $d1;
        if (!empty($d2)) {
            $this->_data[$this->embeddedDataName] = $d2;
        }
    ...
    private function parseParam($param, array $value, array $embedded)
    {
        ...
        if (!empty($embedded)) {
            ...
            $value[$this->embeddedDataName] = $embedded;
        }
        if (empty($data[$param])) {
            if ($allowMultiple) {
                $data[$param] = array($value);
            } else {
                $data[$param] = $value;
            }
            ...
            $arr = array($data[$param], $value);
            $data[$param] = $arr;
        } else {
            if (!is_string($value) && isset($value[$this->embeddedDataName])
                && isset($data[$param][$this->embeddedDataName])
            ) {
                $value[$this->embeddedDataName]
                    += $data[$param][$this->embeddedDataName];
            }
    ...
    private function parseEmbeddedData($subject)
    {
        ...
        while (preg_match($this->embeddedDataPattern, $subject, $matches)) {
            $subject = str_replace($matches[0], '', $subject);
            $str = $matches[$this->embeddedDataIndex];
            if ($str{0} == '{') {
    ...
    private function formatReturn(array $value)
    {
        ...
        $r = array('type' => count($data) == 1 ? $data[0] : $data);
